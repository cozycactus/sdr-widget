...
int set_nvram(int argc, char *argv[]) {
	...
	for (i = true_feature_minor_index+1; i < true_feature_end_index; i += 1) {
		...
		if ((uint8_t)usb_data[0] != features[i]) {
			fprintf(stderr, "widget-control: device rejected %s=%s, returned 0x%02x\n",
					true_feature_index_names[i], true_feature_value_names[features[i]], (unsigned char)usb_data[0]);
			free((void *)features);
			exit(finish(1));
		}
	}
